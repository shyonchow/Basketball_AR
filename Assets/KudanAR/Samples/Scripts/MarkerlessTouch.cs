...
namespace Kudan.AR {
    ...
    public class MarkerlessTouch : MonoBehaviour {
        ...
        /// </summary>
        void Start() {
            LayerHoop = LayerMask.NameToLayer("Hoop");
            moveSpeed = 2f;
            roughDiff = 3f;
            tap = false;
        ...
        void processDrag() {
            if (Input.touchCount == 2) {
                ...
                if (Physics.Raycast(ray, out hitInfo)) {
                    Debug.Log("hitInfo layer: " + hitInfo.transform.gameObject.layer);
                    if (hitInfo.transform.gameObject.layer == LayerHoop) {
                        ...
                        ) //If the finger has moved since the last frame
                        {
                            ...
                            float deltaY = (fingMove.y * moveSpeed * 2);
                            float deltaX = fingMove.x * moveSpeed * -1;
                            interactableObject.transform.Translate(0, deltaX, deltaY);
                        }
                    }
                }
            }
        ...
        void processTap() {
            if (Input.touchCount == 1) {
                ...
                ) //If the finger started touching the screen this frame
                {
                    ...
                    ) //And the finger on the screen is not currently touching an object
                        startPos =
                            fing
                                .position; //Get the screen position of the finger when it hit the screen
                ...
                ) //If the finger stopped touching the screen this frame
                {
                    ...
                    if (Mathf.Abs(endPos.magnitude - startPos.magnitude) < roughDiff
                    ) //Calculate how far away the finger was from its starting point when it left the screen
                    {
                        tap =
                            true; //And if it left the screen roughly where it started, it's a tap
                    }
                }
            }
