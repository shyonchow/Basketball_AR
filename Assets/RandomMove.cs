...
public class RandomMove : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (screenPos.x <= 0.2f * cam.pixelWidth ||
            screenPos.x >= cam.pixelWidth - 0.2f * cam.pixelWidth ||
            screenPos.z >= 600 || ball.count % 2 != 0)
        {
            Debug.Log("Off Screen at: " + screenPos);
            k = 0;
            j = 0;
        }
        else if (ball.count > 0 && r <= 4)
        {
            k = 1;
        }
        else if (ball.count > 0 && r > 4 && r <= 8)
        {
            k = 2;
        }
        else if (ball.count > 0 && r > 8)
        {
            k = 3;
        }
